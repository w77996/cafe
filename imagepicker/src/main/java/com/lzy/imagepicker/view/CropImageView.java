...
import com.lzy.imagepicker.bean.GraphicPath;
import com.lzy.imagepicker.util.LogUtil;
import com.lzy.imagepicker.util.ViewUtil;
...
public class CropImageView extends AppCompatImageView {
    ...
    /******************************** 图片缩放位移控制的参数 ************************************/
    private static final float MAX_SCALE = 2.0f;  //最大缩放比，图片缩放后的大小与中间选中区域的比值
    ...
    private void initImage() {
        ...
        Log.e(TAG, "mRotatedImageWidth : " + mRotatedImageWidth + "  mRotatedImageHeight: " + mRotatedImageHeight);
        ...
        if (mStyle == Style.CIRCLE) {
            ...
            Log.e(TAG, "mFocusWidth : " + mFocusWidth + "  mFocusHeight: " + mFocusHeight);
        }
        ...
        Log.e(TAG, "mFocusRect.left : " + mFocusRect.left + "  mFocusRect.right: "
                + mFocusRect.right + " mFocusRect.top: " + mFocusRect.top + " mFocusRect.bottom: " + mFocusRect.bottom);

        //适配焦点框的缩放比例（图片的最小边不小于焦点框的最小边）
        float fitFocusScale = getScale(mImageWidth, mImageHeight, mFocusWidth, mFocusHeight, true);
        Log.e(TAG, "fitFocusScale : " + fitFocusScale);


        float fitFocusScale2 = getScale(mImageWidth, mImageHeight, mFocusWidth, mFocusHeight, false);
        Log.e(TAG, "fitFocusScale2 : " + fitFocusScale2);

        mMaxScale = fitFocusScale * MAX_SCALE;
        Log.e(TAG, "mMaxScale : " + mMaxScale);
        //适配显示图片的ImageView的缩放比例（图片至少有一边是铺满屏幕的显示的情形）
        float fitViewScale = getScale(mImageWidth, mImageHeight, viewWidth, viewHeight, false);
        Log.e(TAG, "fitViewScale : " + fitViewScale);
...
//        mGraphicPath.clear();
//        for (int i = 0; i <= 360; i++) {
//            mGraphicPath.addPath(midPointX-Math.sin(i),midPointY - Math.cos(i));
//        }
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Style.RECTANGLE == mStyle) {
            ...
            float mR = Math.min(getWidth() / 2, getHeight() / 2);
            mGraphicPath.clear();
            //add circle x,y List
            for (int i = 0; i <= 361; i++) {
                mGraphicPath.addPath((int) (mFocusMidPoint.x + (mR * Math.cos(i * Math.PI / 180)))
                        , (int) (mFocusMidPoint.y + (mR * Math.sin(i * Math.PI / 180))));
//                Log.e(TAG, "x" + i + " : " + (int) (mFocusMidPoint.x + (mR * Math.cos(i * Math.PI / 180))));
//                Log.e(TAG, "y" + i + " : " + (int) (mFocusMidPoint.y + (mR * Math.sin(i * Math.PI / 180))));
            }
...
//                bitmap = Bitmap.createScaledBitmap(bitmap, expectWidth, exceptHeight, true);
     ...
     * @param isSaveRectangle 是否希望按矩形区域保存图片
     */
    public void saveBitmapToFile(Activity activity, File folder, final Bitmap croppedImage
//            , int expectWidth, int exceptHeight
            , boolean isSaveRectangle) {
        if (mSaving) return;
        mSaving = true;
//        final Bitmap croppedImage = getCropBitmap(expectWidth, exceptHeight, isSaveRectangle);
//        final Bitmap croppedImage = getBitmap(activity);
    ...
    private void saveOutput(Bitmap croppedImage, Bitmap.CompressFormat outputFormat, File saveFile) {
        ...
        try {
            ...
            Log.e(TAG, "save error : " + ex.toString());
            ex.printStackTrace();
...
//        croppedImage.recycle();
    ...
    public void getBitmap(Activity activity) {
        mRect = new Rect(mGraphicPath.getLeft(), mGraphicPath.getTop()
                , mGraphicPath.getRight(), mGraphicPath.getBottom());
        Log.e(TAG, "mRect : " + mRect.toString());
        View decorView = activity.getWindow().getDecorView();
        decorView.setDrawingCacheEnabled(true);
        this.buildDrawingCache();
//获取屏幕整张图片
        Bitmap bitmap = decorView.getDrawingCache();
        saveBitmapToFile(activity, cropCacheFolder, toRoundBitmap(bitmap), false);
...
//            height = width;
            dst_left = 0;
//            dst_top = ViewUtil.getStateBarHeight(getContext()) + ViewUtil.dp2px(getContext(), 48);
            dst_top = 0;
            dst_right = width;
            dst_bottom = height;
        } else {
...
//        final Rect dst = new Rect(mRect);
        ...
        canvas.drawCircle(roundPx
                , (height + ViewUtil.getStateBarHeight(getContext()) + ViewUtil.dp2px(getContext(), 48)) / 2+ ViewUtil.getStateBarHeight(getContext())*2/3
                , roundPx-10, paint);
//        canvas.drawCircle(mFocusMidPoint.x
//                , mFocusMidPoint.y
//                , roundPx, paint);
